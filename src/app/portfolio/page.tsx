...
const projects = [
  ...
  },
  {
    ...
    icon: TrendingUp,
    color: "from-purple-200 to-emerald-100",
  ...
  },
  {
    ...
    icon: Palette,
    color: "from-teal-300 to-emerald-100",
    ...
    <>
      ...
      <section className="pt-32 pb-20 bg-gradient-to-br from-emerald-50 to-white text-center relative overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10">
          <Reveal>
            <span className="text-emerald-600 font-semibold tracking-wider uppercase text-sm mb-4 block">Our Work</span>
            ...
            <AnimatePresence>
              {filteredProjects.map((project) => (
                <motion.div
                  ...
                  key={project.id}
                  className="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 group hover:shadow-xl transition-all"
                >
                  <div className={`h-64 bg-gradient-to-br ${project.color} flex items-center justify-center relative overflow-hidden`}>
                    ...
                    <div className="absolute inset-0 bg-emerald-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
                      <button className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-emerald-600 transform scale-0 group-hover:scale-100 transition-transform delay-100">
                        <ExternalLink className="w-5 h-5" />
                      </button>
                    </div>
                  </div>
                  <div className="p-8">
                    <span className="text-emerald-600 font-semibold text-sm mb-2 block">{project.categoryLabel}</span>
                    ...
                    <div className="flex items-center gap-6 pt-6 border-t border-slate-100">
                      {project.stats.map((stat, i) => (
                        <div key={i}>
                          ...
                          <div className="text-xs text-slate-500 font-medium uppercase tracking-wider">{stat.lbl}</div>
                        </div>
                      ))}
                    </div>
                  </div>
